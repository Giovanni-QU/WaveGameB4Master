...
public class Pause {
	...
	// CONSTRUCTOR
	public Pause(HUD hud, Game game, Handler handler, Boolean gameSaved, Spawn1to10 sp1, Spawn10to20 sp2,
			Upgrades upgrade) {
		this.hud = hud;
		this.game = game;
	...
	// METHODS
	public void writeToSavedGameFile(String n, int score, double hp, int level, int enemy, int lvlRem, String ability,
			int abilityUses) {
		...
		try {
			...
			printWriter.print(n + " " + score + " " + hp + " " + level + " " + enemy + " " + lvlRem + " " + ability
					+ " " + abilityUses);
	...
	public void reset() {
		this.pause = new Pause(this.hud, this.game, this.handler, gameSaved, this.spawner1, this.spawner2,
				this.upgrade);
	}
	...
	public void render(Graphics g) {
		...
		if (game.gameState == STATE.PauseShop) {
			...
			int rectW = 1855; //frame.height - 20;
			int rectH = 1000; //frame.width - 20;
			int rectX = 10;
			int rectY = 10;

			g.drawRect(rectX, rectY, rectW, rectH);

			g.drawRect(rectX + 10, rectY + 50, (rectW / 2) - 10, 500);
			// g.drawRect(41, 150, 900, 400);
			// g.drawRect(41 + 900 + 21, 150, 900, 400);

			// g.drawRect(1600, 870, 200, 65); g.drawString("fuck", 1650, 910);
			...
			g.drawString("One you avoid them long enough, a new batch will spawn in! Defeat each boss to win!", 40,
					340);
